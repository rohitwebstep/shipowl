...
async function POST(req) {
    try {
        ...
        const missingVars = Object.entries(requiredEnvVars).filter(([key, val])=>{
        ...
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$app$2f$models$2f$dropshipper$2f$shopify$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createDropshipperShopifyStore"])(mainDropshipperId, String(dropshipperRole), payload);
        if (result?.status) {
            const installUrl = `https://${shop}/admin/oauth/authorize` + `?client_id=${apiKey}` + `&scope=${scopes}` + `&redirect_uri=${encodeURIComponent(redirectUrl)}` + `&grant_options[]=per-user`;
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                status: true,
                installUrl
            }, {
