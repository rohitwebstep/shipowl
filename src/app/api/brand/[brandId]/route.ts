...
import { getBrandById, updateBrand, deleteBrand } from '@/app/models/brand';
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const brandResult = await deleteBrand(brandIdNum);
    if (brandResult?.status) {
      logMessage('info', `Brand deleted successfully: ${brandIdNum}`, { adminId });
      return NextResponse.json({ status: true, message: 'Brand successfully deleted' }, { status: 200 });
    }

    // If brand not found
    logMessage('info', `Brand not found: ${brandIdNum}`, { adminId });
    return NextResponse.json({ status: false, message: 'Brand not found' }, { status: 404 });
  } catch (error) {
